...
export function createAcpClient(cwd, env = {}) {
  ...
  async function initialize() {
    ...
    const result = await send("initialize", {
      ...
      clientInfo: { name: "acp2acpx-bridge", version: "0.1.0" },
    });
    initialized = true;
  ...
  async function* runTurn(existingSessionId, cwdPath, promptText) {
    ...
    const deadline = Date.now() + (Number(process.env.ACP_TURN_TIMEOUT_MS) || 300_000);
    let final = null;
    ...
    yield { type: "error", error: new Error("runTurn timeout") };
  }
