...
export function createAcpClient(cwd, env = {}) {
  ...
  const permissionModeRaw = String(env.OPENCLAW_CURSOR_ACP_PERMISSION_MODE || env.ACP_PERMISSION_MODE || "allow-once").trim().toLowerCase();
  const permissionMode = ["allow-once", "allow-always", "reject-once", "fail"].includes(permissionModeRaw)
    ? permissionModeRaw
    : "allow-once";
  ...
  function resolveOptionId(options) {
    const normalized = Array.isArray(options) ? options : [];
    const byId = (id) => normalized.find((opt) => opt?.optionId === id || opt?.id === id);
    if (permissionMode === "fail" || permissionMode === "reject-once") {
      return byId("reject-once")?.optionId || byId("reject-once")?.id || null;
    }
    if (permissionMode === "allow-always") {
      return byId("allow-always")?.optionId || byId("allow-always")?.id || null;
    }
    // default allow-once
    return byId("allow-once")?.optionId || byId("allow-once")?.id || null;
  }
        ...
        if (!optionId) {
          child.stdin.write(
            JSON.stringify({
              jsonrpc: "2.0",
              id: msg.id,
              error: { code: -32000, message: `permission option not available for mode=${permissionMode}` },
            }) + "\n",
          );
          return;
        }
  ...
  async function sessionSetMode(sessionId, modeId, cwdPath = cwd) {
    if (!sessionId) throw new Error("sessionSetMode requires sessionId");
    if (!modeId) throw new Error("sessionSetMode requires modeId");
    await sessionLoad(sessionId, cwdPath);
    return await withAuthRetry(() => send("session/set_mode", { sessionId, modeId }));
  }

  async function sessionSetConfigOption(sessionId, configId, valueId, cwdPath = cwd) {
    if (!sessionId) throw new Error("sessionSetConfigOption requires sessionId");
    if (!configId || !valueId) throw new Error("sessionSetConfigOption requires configId/valueId");
    await sessionLoad(sessionId, cwdPath);
    return await withAuthRetry(() => send("session/set_config_option", { sessionId, configId, valueId }));
  }
  ...
  function parseOptionalCount(raw, fallback = 1) {
  ...
  async function* runTurn(existingSessionId, cwdPath, promptText) {
    ...
    const idleAutoRecover = /^(1|true|yes)$/i.test(String(process.env.ACP_IDLE_AUTO_RECOVER || ""));
    const idleRecoverMax = parseOptionalCount(process.env.ACP_IDLE_RECOVER_MAX, 1);
    let idleRecoverCount = 0;
    ...
    while (true) {
      ...
      if (idleTimeoutMs > 0 && now - lastProgressAt >= idleTimeoutMs) {
        if (idleAutoRecover && idleRecoverCount < idleRecoverMax) {
          idleRecoverCount += 1;
          try {
            await sessionLoad(sessionId, cwdPath);
            lastProgressAt = Date.now();
            yield {
              ...
              params: {
                ...
                update: {
                  ...
                  content: { type: "text", text: `[bridge] idle recovery attempt ${idleRecoverCount}/${idleRecoverMax}` },
                },
              },
            };
            continue;
          } catch (recoverErr) {
            yield { type: "error", error: recoverErr };
            return;
          }
        }
